...
def main(page:Page):
    ...
    category_input = TextField(hint_text="Enter new category",color=colors.GREEN_900, expand=True, on_submit=lambda _:add_category())
...
#drop down for editdata page
    ...
    analysis_dropdown=Dropdown(
        ...
        on_change=lambda _ : change_analysis_format(),color=textlist[1] if dark_mode == False else textlist[0]
    )   

    Input_Amount=TextField(hint_text="Enter amount", expand=True, on_submit=lambda _:add_expense(),color=colors.GREEN_900)
    Input_Amount1=TextField(hint_text="Enter amount", expand=True, on_submit=lambda _:add_expense1(),color=colors.GREEN_900)
    ...
    Input_username=TextField(hint_text="Enter username...",on_submit=lambda _:username_submit(),color=cardlist[1] if dark_mode == False else cardlist[0],height=210)
#####################################################################################################################
# initializing pages
...
#####################################################################################################################
    ...
    Home_Page_Contents=Row(                                                         #DAILY GRAPHS WILL APPEAR IN HOME PAGE
        controls=[Container(expand=True,
                        bgcolor = bglist[1] if dark_mode == False else bglist[0],
                                                ...
                                                Row(#date picker
                                                    ...
                                                    controls=[
                                                        Card(
                                                            width=210,
                                                            height=50,
                                                            color=bglist[1] if dark_mode == False else bglist[0],
                                                            content=(
                                                                Row(
                                                                    controls=[
                                                                        IconButton(
                                                                            width=30,
                                                                            content=Icon(icons.ARROW_BACK_IOS,color=cardlist[1] if dark_mode == False else cardlist[0]),
                                                                        ...
                                                                        IconButton(
                                                                            width=30,
                                                                            content=Icon(icons.ARROW_FORWARD_IOS,color=cardlist[1] if dark_mode == False else cardlist[0]),
                                                        ...
                                                        content=(
                                                            Row(
                                                                controls=[
                                                                    ...
                                                                    Text(get_week_dates(database.get_week_no(datetime.datetime.today().strftime("%d %m %Y"))),color=cardlist[1] if dark_mode == False else cardlist[0]),
                                                                    IconButton(
                                                                        width=30,
                                                                        content=Icon(icons.ARROW_FORWARD_IOS,color=cardlist[1] if dark_mode == False else cardlist[0]),
                        ...
                        content=Column(controls=[
                            Row(alignment=MainAxisAlignment.CENTER,
                                ...
                                Card(
                                    ...
                                    elevation=1,
                                    color=bglist[1] if dark_mode == False else bglist[0],
                                    content=Text(value=datetime.datetime.today().strftime('%d/%m/%Y'),color=cardlist[1] if dark_mode == False else cardlist[0],text_align=TextAlign.CENTER,weight=FontWeight.BOLD,size=20),
                                )
                            ]
                            ),
                            Row(alignment=MainAxisAlignment.CENTER,
                           ...
                           content=ListView(controls=[
                                ...
                                controls=[
                                    Card(
                                        width=210,
                                        height=50,
                                        color=cardlist[1] if dark_mode == False else bglist[0],
                                        content=(
                                            Row(
                                                controls=[
                                                    IconButton(
...
#                           alignment=alignment.top_right
    ...
    def add_category_row(category, color):
        category_container.controls.append(Container(bgcolor=cardlist[1] if dark_mode == False else cardlist[0], border_radius=15, padding=5, content=
            Row(
    ...
    def add_editdata_row(category_color,category,amount,date):
            editdata_list.controls.append(
                ...
                controls=[
                    ...
                    Text(category, color=cardlist[1] if dark_mode == False else cardlist[0],size=20),
                    Text(amount, color=cardlist[1] if dark_mode == False else cardlist[0],size=20),
                    Row(
                        controls=[
                            IconButton(
                                content=Icon(icons.EDIT),
                    ...
                    Container(
                        ...
                        content=(
                            Row(
                                alignment=MainAxisAlignment.SPACE_BETWEEN,
                                controls=[
                                    Text("  categories",italic=True,color=cardlist[1] if dark_mode == False else cardlist[0],size=20,),
                                    Text("amount    ",italic=True,color=cardlist[1] if dark_mode == False else cardlist[0],size=20,)
                                ]
                            )
                        )
                    )
                    ...
                    Row(
                                                                controls=[
                                                                    Container(
                                                                        ...
                                                                        border_radius=8,
                                                                        bgcolor=cardlist[1] if dark_mode == False else cardlist[0],
                                                                        content=(
                                                                            Row(
                                                                                alignment=MainAxisAlignment.SPACE_BETWEEN,
                                                                                controls=[
            ...
            Row(
                controls=[
                    Container(
                        expand=True,
                        height=50,
                        #color=cardlist[1] if dark_mode == False else cardlist[0],
                        content=(
                            Row(
                                alignment=MainAxisAlignment.SPACE_BETWEEN,
                                controls=[
                    ...
                    Container(
                        ...
                        content=(
                            Row(
                                alignment=MainAxisAlignment.SPACE_BETWEEN,
                                controls=[
                                    Text("  categories",italic=True,size=20,color=cardlist[1] if dark_mode == False else cardlist[0]),
                                    Text("amount    ",italic=True,size=20,color=cardlist[1] if dark_mode == False else cardlist[0])
                                ]
                            )
                        )
                    )
    ...
    def change_mode():
        ...
        check_dark()
        Home_Page_Contents.controls[0].bgcolor=bglist[1] if dark_mode == False else bglist[0]
    ...
    pages={'/':View('/',[Initial_Page],floating_action_button = FloatingActionButton(icon=icons.ADD_OUTLINED,
               ...
               floating_action_button_location = FloatingActionButtonLocation.CENTER_DOCKED,
                ...
                end_drawer = NavigationDrawer(
                            ...
                            IconButton(
                                content=Icon(icons.DARK_MODE_OUTLINED),
                                on_click=lambda _:change_mode()
                            )
                            ]
                ),
           ...
           '/AddData_Contents':View('/AddData_Contents',[Container(expand=True,
            ...
            '/Login':View('/Login',[Container(padding=20,bgcolor=bglist[1] if dark_mode == False else bglist[0],expand=True,
                                            animate=animation.Animation(600, AnimationCurve.DECELERATE),
                            content=Column(
                                alignment=MainAxisAlignment.CENTER,
                                horizontal_alignment=CrossAxisAlignment.CENTER,
                                controls=[Text(value='Login Page',text_align='center',size=60,color=cardlist[1] if dark_mode == False else cardlist[0],weight=FontWeight.BOLD),
                                    Input_username,
                                ]
                            ))])
                }
...
#####################################################################################################################
    ...
    page.theme=Theme(color_scheme_seed=colors.GREEN_900) if dark_mode==False else Theme(color_scheme_seed=colors.GREEN_600)
